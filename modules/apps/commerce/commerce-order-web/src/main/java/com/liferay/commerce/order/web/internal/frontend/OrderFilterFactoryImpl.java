...
@Component(
	...
	property = {
		...
		"commerce.data.provider.key=" + CommerceOrderDataSetConstants.COMMERCE_DATA_SET_KEY_TRANSMITTED_ORDERS
	},
...
public class OrderFilterFactoryImpl implements FilterFactory {
	...
	public Filter create(HttpServletRequest httpServletRequest) {
		...
		String advanceStatus = ParamUtil.getString(
			httpServletRequest, "advanceStatus");
		String keywords = ParamUtil.getString(httpServletRequest, "keywords");
		int orderStatus = ParamUtil.getInteger(
			httpServletRequest, "orderStatus",
			CommerceOrderConstants.ORDER_STATUS_ANY);

		orderFilterImpl.setAdvanceStatus(advanceStatus);
		orderFilterImpl.setKeywords(keywords);
		orderFilterImpl.setOrderStatus(orderStatus);
