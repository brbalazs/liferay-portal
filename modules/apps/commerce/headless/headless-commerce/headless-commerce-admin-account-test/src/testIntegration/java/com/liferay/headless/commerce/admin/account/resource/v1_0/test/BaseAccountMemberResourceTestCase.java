...
public abstract class BaseAccountMemberResourceTestCase {
	...
	protected void assertEqualsJSONArray(
		List<AccountMember> accountMembers, JSONArray jsonArray) {

		for (AccountMember accountMember : accountMembers) {
			boolean contains = false;

			for (Object object : jsonArray) {
				if (equalsJSONObject(accountMember, (JSONObject)object)) {
					contains = true;
			...
			Assert.assertTrue(
				jsonArray + " does not contain " + accountMember, contains);
		}
	}
	...
	protected boolean equalsJSONObject(
		AccountMember accountMember, JSONObject jsonObject) {

		for (String fieldName : getAdditionalAssertFieldNames()) {
			if (Objects.equals("accountId", fieldName)) {
				if (!Objects.deepEquals(
						accountMember.getAccountId(),
			...
			if (Objects.equals("userExternalReferenceCode", fieldName)) {
				if (!Objects.deepEquals(
						accountMember.getUserExternalReferenceCode(),
						jsonObject.getString("userExternalReferenceCode"))) {
		...
		return new AccountMember() {
			{
				...
				userExternalReferenceCode = RandomTestUtil.randomString();
