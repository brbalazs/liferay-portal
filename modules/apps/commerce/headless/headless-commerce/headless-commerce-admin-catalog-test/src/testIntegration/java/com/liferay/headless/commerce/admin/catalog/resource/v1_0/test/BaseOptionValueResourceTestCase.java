...
public abstract class BaseOptionValueResourceTestCase {
	...
	protected void assertEqualsJSONArray(
		List<OptionValue> optionValues, JSONArray jsonArray) {

		for (OptionValue optionValue : optionValues) {
			boolean contains = false;

			for (Object object : jsonArray) {
				if (equalsJSONObject(optionValue, (JSONObject)object)) {
					contains = true;
			...
			Assert.assertTrue(
				jsonArray + " does not contain " + optionValue, contains);
		}
	}
	...
	protected boolean equals(
		...
		for (String additionalAssertFieldName :
			...
			if (Objects.equals("name", additionalAssertFieldName)) {
				if (!Objects.deepEquals(
						optionValue1.getName(), optionValue2.getName())) {
	...
	protected boolean equalsJSONObject(
		OptionValue optionValue, JSONObject jsonObject) {
		...
		for (String fieldName : getAdditionalAssertFieldNames()) {
			...
			if (Objects.equals("id", fieldName)) {
				if (!Objects.deepEquals(
						optionValue.getId(), jsonObject.getLong("id"))) {
			...
			if (Objects.equals("key", fieldName)) {
				if (!Objects.deepEquals(
						optionValue.getKey(), jsonObject.getString("key"))) {
