...
public abstract class BaseDiscountProductResourceTestCase {
	...
	public void testGraphQLDeleteDiscountProduct() throws Exception {
		...
		GraphQLField graphQLField = new GraphQLField(
			...
			new GraphQLField(
				...
				new HashMap<String, Object>() {
					{
						put("discountProductId", discountProduct.getId());
					}
				}));
		...
		Assert.assertTrue(dataJSONObject.getBoolean("deleteDiscountProduct"));
	...
	protected void assertEqualsJSONArray(
		List<DiscountProduct> discountProducts, JSONArray jsonArray) {

		for (DiscountProduct discountProduct : discountProducts) {
			boolean contains = false;

			for (Object object : jsonArray) {
				if (equalsJSONObject(discountProduct, (JSONObject)object)) {
					contains = true;
			...
			Assert.assertTrue(
				jsonArray + " does not contain " + discountProduct, contains);
		}
	}
	...
	protected boolean equalsJSONObject(
		DiscountProduct discountProduct, JSONObject jsonObject) {
		...
		for (String fieldName : getAdditionalAssertFieldNames()) {
			...
			if (Objects.equals("id", fieldName)) {
				if (!Objects.deepEquals(
						discountProduct.getId(), jsonObject.getLong("id"))) {
			...
			if (Objects.equals("productExternalReferenceCode", fieldName)) {
				if (!Objects.deepEquals(
		...
		return new DiscountProduct() {
			{
				...
				productExternalReferenceCode = RandomTestUtil.randomString();
