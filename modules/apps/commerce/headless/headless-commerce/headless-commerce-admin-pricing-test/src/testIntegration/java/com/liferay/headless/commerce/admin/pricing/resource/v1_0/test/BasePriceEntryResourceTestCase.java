...
public abstract class BasePriceEntryResourceTestCase {
	...
	public void testGraphQLDeletePriceEntry() throws Exception {
		...
		Assert.assertTrue(dataJSONObject.getBoolean("deletePriceEntry"));
	...
	public void testGraphQLGetPriceEntry() throws Exception {
		...
		Assert.assertTrue(
			equalsJSONObject(
				priceEntry, dataJSONObject.getJSONObject("priceEntry")));
	}
	...
	protected void assertEqualsJSONArray(
		List<PriceEntry> priceEntries, JSONArray jsonArray) {

		for (PriceEntry priceEntry : priceEntries) {
			boolean contains = false;

			for (Object object : jsonArray) {
				if (equalsJSONObject(priceEntry, (JSONObject)object)) {
					contains = true;
			...
			Assert.assertTrue(
				jsonArray + " does not contain " + priceEntry, contains);
		}
	}
		...
		for (String additionalAssertFieldName :
			...
			if (Objects.equals("customFields", additionalAssertFieldName)) {
				if (!Objects.deepEquals(
						priceEntry1.getCustomFields(),
						priceEntry2.getCustomFields())) {
	...
	protected boolean equalsJSONObject(
		PriceEntry priceEntry, JSONObject jsonObject) {
		...
		for (String fieldName : getAdditionalAssertFieldNames()) {
			...
			if (Objects.equals("hasTierPrice", fieldName)) {
				if (!Objects.deepEquals(
						priceEntry.getHasTierPrice(),
						jsonObject.getBoolean("hasTierPrice"))) {
			...
			if (Objects.equals("id", fieldName)) {
				if (!Objects.deepEquals(
						priceEntry.getId(), jsonObject.getLong("id"))) {
			...
			if (Objects.equals("priceListExternalReferenceCode", fieldName)) {
				if (!Objects.deepEquals(
			...
			if (Objects.equals("priceListId", fieldName)) {
				if (!Objects.deepEquals(
						priceEntry.getPriceListId(),
						jsonObject.getLong("priceListId"))) {
			...
			if (Objects.equals("sku", fieldName)) {
				if (!Objects.deepEquals(
						priceEntry.getSku(), jsonObject.getString("sku"))) {
			...
			if (Objects.equals("skuExternalReferenceCode", fieldName)) {
				if (!Objects.deepEquals(
						priceEntry.getSkuExternalReferenceCode(),
						jsonObject.getString("skuExternalReferenceCode"))) {
			...
			if (Objects.equals("skuId", fieldName)) {
				if (!Objects.deepEquals(
						priceEntry.getSkuId(), jsonObject.getLong("skuId"))) {
		...
		return new PriceEntry() {
			{
				...
				priceListExternalReferenceCode = RandomTestUtil.randomString();
				priceListId = RandomTestUtil.randomLong();
				sku = RandomTestUtil.randomString();
				skuExternalReferenceCode = RandomTestUtil.randomString();
