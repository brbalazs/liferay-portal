...
public abstract class BasePriceListResourceTestCase {
	...
	public void testGraphQLGetPriceListsPage() throws Exception {
		...
		JSONObject priceListsJSONObject = dataJSONObject.getJSONObject(
			"priceLists");
		...
		priceListsJSONObject = dataJSONObject.getJSONObject("priceLists");

		Assert.assertEquals(2, priceListsJSONObject.get("totalCount"));

		assertEqualsJSONArray(
			Arrays.asList(priceList1, priceList2),
			priceListsJSONObject.getJSONArray("items"));
	}
	...
	public void testGraphQLDeletePriceList() throws Exception {
		...
		Assert.assertTrue(dataJSONObject.getBoolean("deletePriceList"));
	...
	public void testGraphQLGetPriceList() throws Exception {
		...
		Assert.assertTrue(
			equalsJSONObject(
				priceList, dataJSONObject.getJSONObject("priceList")));
	}
	...
	protected void assertEqualsJSONArray(
		List<PriceList> priceLists, JSONArray jsonArray) {

		for (PriceList priceList : priceLists) {
			boolean contains = false;

			for (Object object : jsonArray) {
				if (equalsJSONObject(priceList, (JSONObject)object)) {
					contains = true;
			...
			Assert.assertTrue(
				jsonArray + " does not contain " + priceList, contains);
		}
	}
	...
	protected boolean equals(PriceList priceList1, PriceList priceList2) {
		...
		for (String additionalAssertFieldName :
			...
			if (Objects.equals("customFields", additionalAssertFieldName)) {
				if (!Objects.deepEquals(
						priceList1.getCustomFields(),
						priceList2.getCustomFields())) {
	...
	protected boolean equalsJSONObject(
		PriceList priceList, JSONObject jsonObject) {
		...
		for (String fieldName : getAdditionalAssertFieldNames()) {
			...
			if (Objects.equals("id", fieldName)) {
				if (!Objects.deepEquals(
						priceList.getId(), jsonObject.getLong("id"))) {
			...
			if (Objects.equals("name", fieldName)) {
				if (!Objects.deepEquals(
						priceList.getName(), jsonObject.getString("name"))) {
