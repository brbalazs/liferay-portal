...
public abstract class BaseTierPriceResourceTestCase {
	...
	public void testGraphQLGetTierPrice() throws Exception {
		...
		Assert.assertTrue(
			equalsJSONObject(
				tierPrice, dataJSONObject.getJSONObject("tierPrice")));
	}
	...
	protected void assertEqualsJSONArray(
		List<TierPrice> tierPrices, JSONArray jsonArray) {

		for (TierPrice tierPrice : tierPrices) {
			boolean contains = false;

			for (Object object : jsonArray) {
				if (equalsJSONObject(tierPrice, (JSONObject)object)) {
					contains = true;
			...
			Assert.assertTrue(
				jsonArray + " does not contain " + tierPrice, contains);
		}
	}
		...
		for (String additionalAssertFieldName :
			...
			if (Objects.equals("customFields", additionalAssertFieldName)) {
				if (!Objects.deepEquals(
						tierPrice1.getCustomFields(),
						tierPrice2.getCustomFields())) {
	...
	protected boolean equalsJSONObject(
		TierPrice tierPrice, JSONObject jsonObject) {
		...
		for (String fieldName : getAdditionalAssertFieldNames()) {
			...
			if (Objects.equals("id", fieldName)) {
				if (!Objects.deepEquals(
						tierPrice.getId(), jsonObject.getLong("id"))) {
			...
			if (Objects.equals("minimumQuantity", fieldName)) {
				if (!Objects.deepEquals(
						tierPrice.getMinimumQuantity(),
						jsonObject.getInt("minimumQuantity"))) {
			...
			if (Objects.equals("priceEntryExternalReferenceCode", fieldName)) {
				if (!Objects.deepEquals(
			...
			if (Objects.equals("priceEntryId", fieldName)) {
				if (!Objects.deepEquals(
						tierPrice.getPriceEntryId(),
						jsonObject.getLong("priceEntryId"))) {
		...
		return new TierPrice() {
			{
				...
				priceEntryExternalReferenceCode = RandomTestUtil.randomString();
