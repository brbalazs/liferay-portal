...
public class AssetPermissionResourceTest
	extends BaseAssetPermissionResourceTestCase {
	...
	public void setUp() throws Exception {
		...
		_randomString = RandomTestUtil.randomString();
		...
		Bundle testBundle = FrameworkUtil.getBundle(
			AssetPermissionResourceTest.class);
	...
	public void testPutAssetPermissionReset() throws Exception {
		...
		jsonObject.put(
			ObjectEntryFolderConstants.EXTERNAL_REFERENCE_CODE_CONTENTS,
			_getRolePermissionsJSONObject()
		).put(
			ObjectEntryFolderConstants.EXTERNAL_REFERENCE_CODE_FILES,
			_getRolePermissionsJSONObject()
		).put(
			"OBJECT_ENTRY_FOLDERS", _getRolePermissionsJSONObject()
		);
		...
		_verifyInheritedObjectEntryPermissions(objectEntry2);
		_changeObjectEntryPermissions(objectEntry2);
		_resetObjectEntryPermissions(objectEntry2);
		...
		_verifyInheritedObjectEntryFolderPermissions(objectEntryFolder3);
		_changeObjectEntryFolderPermissions(objectEntryFolder3);
		_resetObjectEntryFolderPermissions(objectEntryFolder3);
	}

	private void _changeObjectEntryFolderPermissions(
			ObjectEntryFolder objectEntryFolder3)
		throws Exception {
		...
		_resourcePermissionLocalService.setResourcePermissions(
			...
			ResourceConstants.SCOPE_INDIVIDUAL,
			String.valueOf(objectEntryFolder3.getObjectEntryFolderId()),
			_userRole.getRoleId(), new String[] {ActionKeys.UPDATE});
		...
		Assert.assertFalse(resourcePermission.hasActionId(_randomString));
	...
	private void _changeObjectEntryPermissions(ObjectEntry objectEntry)
		throws Exception {

		_resourcePermissionLocalService.setResourcePermissions(
			objectEntry.getCompanyId(), objectEntry.getModelClassName(),
			ResourceConstants.SCOPE_INDIVIDUAL,
			String.valueOf(objectEntry.getPrimaryKeyObj()),
			_cmsAdministratorRole.getRoleId(),
			new String[] {ActionKeys.DELETE, ActionKeys.VIEW});
	...
	private JSONObject _getRolePermissionsJSONObject() {
		return JSONUtil.put(
			RoleConstants.CMS_ADMINISTRATOR,
			JSONUtil.putAll(ActionKeys.UPDATE, ActionKeys.VIEW, _randomString)
		).put(
	...
	private void _resetObjectEntryFolderPermissions(
			ObjectEntryFolder objectEntryFolder3)
		throws Exception {

		assetPermissionResource.putAssetPermissionReset(
	...
	private void _resetObjectEntryPermissions(ObjectEntry objectEntry)
		throws Exception {
	...
	private void _verifyInheritedObjectEntryFolderPermissions(
			ObjectEntryFolder objectEntryFolder3)
		throws Exception {
	...
	private void _verifyInheritedObjectEntryPermissions(ObjectEntry objectEntry)
		throws Exception {
