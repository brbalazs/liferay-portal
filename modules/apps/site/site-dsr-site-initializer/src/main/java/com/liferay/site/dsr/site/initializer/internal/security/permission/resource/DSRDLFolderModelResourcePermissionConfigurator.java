...
public class DSRDLFolderModelResourcePermissionConfigurator
	implements ModelResourcePermissionFactory.
				   ModelResourcePermissionConfigurator<DLFolder> {

	@Override
	public void configureModelResourcePermissionLogics(
		ModelResourcePermission<DLFolder> modelResourcePermission,
		Consumer<ModelResourcePermissionLogic<DLFolder>> consumer) {

		consumer.accept(
			(permissionChecker, name, dlFolder, actionId) -> {
				if (_delegableActionIds.contains(actionId)) {
					return null;
				}

				if (DSRRoomUtil.isReadOnly(
						dlFolder.getGroupId(), permissionChecker)) {
