...
User commerceAccountUser = (User)row.getObject();

String editURL = commerceAccountMembersDisplayContext.getEditURL(commerceAccountUser);
%>
...
>
	...
	<c:choose>
		<c:when test="<%= commerceAccountUser.isActive() %>">
			<portlet:actionURL name="editUser" var="deactivateURL">
				<portlet:param name="<%= Constants.CMD %>" value="<%= Constants.DEACTIVATE %>" />
				<portlet:param name="redirect" value="<%= currentURL %>" />
				<portlet:param name="userId" value="<%= String.valueOf(commerceAccountUser.getUserId()) %>" />
			</portlet:actionURL>
