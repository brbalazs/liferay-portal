...
function upgrade_identityactivitysummary_table {
	local asah_project_id=${1}

	sed -e "s/\${asah_project_id}/$asah_project_id/g" -e "s/\${PROJECT_ID}/$PROJECT_ID/g" -e "s/\${REGION_ID}/$REGION_ID/g" upgrade_identityactivitysummary_table.sql > new_upgrade_identityactivitysummary_table.sql

	echo "Upgrade Identity Activity Summary Table for Project ID: ${PROJECT_ID}, Asah Project ID: ${asah_project_id}"

	bq --project_id ${PROJECT_ID} query --use_legacy_sql=false < new_upgrade_identityactivitysummary_table.sql
}
...
do :
	upgrade_identityactivitysummary_table ${i}
