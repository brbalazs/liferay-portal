 ...
 */
@Component
public class IndividualUpgradeStep implements UpgradeStep {
	...
	public void upgrade(String version) {
		_updateTableFields(
			Arrays.asList(
				Field.newBuilder(
					"suppressed", LegacySQLTypeName.BOOLEAN
				).setMode(
					Field.Mode.NULLABLE
				).build()),
			"individual");
		...
		if (_log.isInfoEnabled()) {
			_log.info("Individual table successfully upgraded to schema 4.2.0");
		}
	}
