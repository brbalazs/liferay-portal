...
const TimeZonePicker: React.FC<ITimeZonePicker> = ({
    ...
    const [selectedCountry, setSelectedCountry] = useState<string>(initialTimeZone && initialTimeZone.country);

	const getCountries = (): Array<string> => (
		timezonesAvailable ? Array.from(new Set(timezonesAvailable.map(timeZone => timeZone.country).sort())) : []
    );
